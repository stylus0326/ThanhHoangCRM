...
namespace CRM
{
    sealed class NotificationManager
    {
        ...
        public static void Show(Control Parent, string Message, bool sus = true, int timein = 1500)
        {
            if (!(isVisible))
            {
                isVisible = true;
                ...
                if (sus)
                {
                    GlowColor = Color.FromArgb(0, 128, 0);
                    BackGroundColor = Color.FromArgb(0, 128, 64);
                }
                else
                    GlowColor = BackGroundColor = Color.Crimson;
