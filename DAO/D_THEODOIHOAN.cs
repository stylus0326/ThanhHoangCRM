...
{
    public class D_THEODOIHOAN : DataProcess
    {
        public List<O_THEODOIHOAN> DuLieu(DateTime Tu, DateTime Den, int LoaiKhach, bool All)
        {
            return LayDuLieu<O_THEODOIHOAN>(false, string.Format(@"declare @TuNgay date = '{0}'
...
{3}", Tu.ToString("yyyyMMdd"), Den.ToString("yyyyMMdd"), LoaiKhach, All ? "" : "where coalesce(IDKhachHang,0) + coalesce(NH.MaDL,0) >0"));
        ...
        public List<O_THEODOIHOAN> DuLieuNCC(DateTime Tu, DateTime Den, bool All)
        {
            return LayDuLieu<O_THEODOIHOAN>(false, string.Format(@"declare @TuNgay date = '{0}'
declare @DenNgay date = '{1}'
...
,coalesce(GiaNet,0)+coalesce(GiaNetKS,0) GiaDuong,coalesce(HangHoan,0)+coalesce(SoTienBaoLuu,0) GiaAm , 0-coalesce(NopTien,0) + coalesce(SoTienDuong,0) 'NopTien',coalesce(SoTienAm,0) + coalesce(RutTien,0) 'RutTien'
,(case when coalesce(SoDuCuoiCoDinh,0) >0 then coalesce(SoDuCuoiCoDinh,0) else 0 end) 'SoDuCuoiDuong',(case when coalesce(SoDuCuoiCoDinh,0) < 0 then 0 - coalesce(SoDuCuoiCoDinh,0) else 0 end) 'SoDuCuoiAm'
...
LEFT JOIN (SELECT KhachSan,COALESCE(sum(SoTienBaoLuu),0) SoTienBaoLuu, COALESCE(sum(Gianet),0) GianetKS  from KHACHSAN 
							WHERE CONVERT(date, NgayGD) between @TuNgay and @DenNgay AND COALESCE(KhachSan,0)>0 
							GROUP BY KhachSan) G5 
							on SD2.NCCID =  G5.KhachSan
...
{2}", Tu.ToString("yyyyMMdd"), Den.ToString("yyyyMMdd"), All ? "" : @"where coalesce(NhaCungCap,0) + coalesce(NH.MaDL,0)+ coalesce(G5.KhachSan,0) >0"));
        }

        public List<O_THEODOIHOAN> DuLieuNH(DateTime Tu, DateTime Den, bool All)
        {
            return LayDuLieu<O_THEODOIHOAN>(false, string.Format(@"declare @TuNgay date = '{0}'
declare @DenNgay date = '{1}'
