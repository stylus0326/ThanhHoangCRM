...
namespace DataAccessLayer
{
    public class DaiLyD : DataProcess
    {
        ...
        public List<DaiLyO> KhachLe()
        {

            return LayDuLieu<DaiLyO>(false, string.Format(@"SELECT DL.*,(case when CONVERT(date,NgayKiQuy)>CONVERT(date,GETDATE()-30) then N'Mới' else COALESCE(Nhom,'-') end ) Nhom,GiaoDichGanNhat 
FROM DAILY dl
...
WHERE LoaiKhachHang = 3 order by GiaoDichGanNhat desc"));
        ...
        public List<DaiLyO> NhanVien()
        {
            return LayDuLieu<DaiLyO>(false, "SELECT * FROM DAILY WHERE LoaiKhachHang = 0");
        }

        public List<DaiLyO> CongNo(int LoaiKhach, DateTime day1, DateTime day2)
        {
            return LayDuLieu<DaiLyO>(false, string.Format(@"SELECT ID, EmailKeToan, Ten, SoDuCuoi, MaDL FROM DAILY WHERE LoaiKhachHang = {2} and id in (select distinct IDKhachHang from GIAODICH where convert(date,NgayGD) BETWEEN '{0}' and '{1}') ORDER BY Ten", day1.ToString("yyyy/M/d"), day2.ToString("yyyy/M/d"), LoaiKhach));
        }

        public List<DaiLyO> HoaDon(DateTime day1, DateTime day2)
        {
            return LayDuLieu<DaiLyO>(false, string.Format(@"SELECT ID, EmailKeToan, Ten, SoDuCuoi, MaDL,LoaiKhachHang FROM DAILY WHERE id in (select distinct IDKhachHang from HOADON where convert(date,NgayThucHien) BETWEEN '{0}' and '{1}' AND MaHD <> '0' AND ((GiaYeuCau - GiaHeThong) * PhanTram / 100) > 0) and LoaiKhachHang in (1,2) ORDER BY Ten", day1.ToString("yyyyMMdd"), day2.ToString("yyyyMMdd")));
        }

        public DaiLyO DangNhap(string ten, string mk)
        {
            return LayMotDongDonGian<DaiLyO>(string.Format(@"WHERE TenDangNhapCty = '{0}' AND MatKhauCty = '{1}'", ten, mk));
        }

        public List<DaiLyO> All(bool KL = true)
        {
            List<DaiLyO> lst = new List<DaiLyO>();
            lst.Add(new DaiLyO() { ID = -1, Ten = "-" });
            lst.Add(new DaiLyO() { ID = 0, Ten = "-" });
            string CRV = string.Format("SELECT ID, Ten, MaDL,LoaiKhachHang,EmailGiaoDich,EmailKeToan,NgayKiQuy,MaAGS FROM DAILY") + (KL ? "" : " where LoaiKhachHang <3");
            lst.AddRange(LayDuLieu<DaiLyO>(false, CRV));
            return lst;
        }

        public List<DaiLyO> DuLieu(int Loai)
        {
            string ctv = @"SELECT DL.*,(case when CONVERT(date,NgayKiQuy)>CONVERT(date,GETDATE()-30) then N'Mới' else COALESCE(Nhom,'-') end ) Nhom,GiaoDichGanNhat,QuyChet ,SoDu
FROM DAILY dl
{0}
...
WHERE LoaiKhachHang = {1} and Nghi = 0 order by GiaoDichGanNhat desc";
            ...
            return LayDuLieu<DaiLyO>(false, ctv);
        }

        public enum Get { XoaAn, IDName }

        public List<DaiLyO> LayDanhSach(Get LayKieu, int LoaiKhach = 0, DateTime day1 = default(DateTime), DateTime day2 = default(DateTime))
        {
            ...
            lst.AddRange(LayDuLieu<DaiLyO>(false, CTV));
            return lst;
        }

        public DaiLyO LayDaiLy(bool ID, string KieuDL)
        {
            string CTV = string.Empty;
            if (ID)
            ...
            return LayMotDongDonGian<DaiLyO>(CTV);
        }
        #endregion
        ...
        public List<DaiLyO> LayDanhSachNo3NgayLienTiep(DateTime Ngay)
        {
            return LayDuLieu<DaiLyO>(false, string.Format(@"declare @Ngay datetime= '{0}' 
...
UPDATE SODU_DAILY SET SoDuCuoi = Sd.SoDu from 
    ...
    public class TinhTrangD : DataProcess
    {
        public TinhTrangD()
        {
            TableName = "TRANGTHAI";
        ...
        public List<TinhTrangO> DuLieu(int LoaiKhach, bool TatCa = false)
        {
            if (TatCa)
                return LayDuLieu<TinhTrangO>();
            else
                return LayDuLieu<TinhTrangO>(true, string.Format(@"WHERE LoaiKhachHang = {0}", LoaiKhach));
        }
    }

    public class NhomDaiLyD : DataProcess
    {
        public NhomDaiLyD()
        {
            TableName = "NHOMDAILY";
        ...
        public List<NhomDaiLyO> DuLieu(int LoaiKhach, bool TatCa = false)
        {
            if (TatCa)
                return LayDuLieu<NhomDaiLyO>();
            else
                return LayDuLieu<NhomDaiLyO>(true, string.Format(@"WHERE LoaiKhachHang = {0}", LoaiKhach)); ;
        }
