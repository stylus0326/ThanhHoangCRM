...
{
    public class HoaDonD : DataProcess
    {
        public HoaDonD()
        {
            TableName = "HOADON";
        ...
        #region dùng
        public List<HoaDonO> DuLieu(string a)
        {
            return LayDuLieu<HoaDonO>(true, "WHERE " + a + (a.Contains("ORDER") ? "" : "ORDER BY SoChungTu"), ",GiaYeuCau - GiaHeThong 'CL1', (GiaYeuCau - GiaHeThong) * PhanTram / 100 'CL2', (GiaYeuCau - GiaHeThong) * 10 / 11 'CL5', (case when GiaHeThong2 > 0 then GiaYeuCau - GiaHeThong2 else 0 end) 'CL3', (case when GiaHeThong2 > 0 then (GiaYeuCau - GiaHeThong2) * PhanTram2 / 100 else 0 end) 'CL4', (case when GiaHeThong2 > 0 then (GiaYeuCau - GiaHeThong2) * 10 / 11 else 0 end) 'CL6'");
        }

        public List<HoaDonO> LayThongTinMST()
        {
            return LayDuLieu<HoaDonO>(false, "SELECT MaSoThue, CongTy, DiaChi, Mail, IDKhachHang FROM HOADON GROUP BY MaSoThue, CongTy, DiaChi, Mail, IDKhachHang");
        }
